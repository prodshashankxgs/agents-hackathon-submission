...
const api = axios.create({
  ...
  timeout: 90000, // 90 second timeout for API requests (13F calls can take longer)
  headers: {
...
export const apiService = {
  ...
  create13FBasket(institution: string, investmentAmount: number, options?: {
    maxPositions?: number
    minWeight?: number
    rebalanceThreshold?: number
  }): Promise<{ success: boolean; basket: PortfolioBasket; message: string }> {
    return api.post('/13f/create-basket', { 
      institution, 
      investmentAmount, 
      options 
    }).then(res => res.data)
  ...
  execute13FBasket(basketId: string): Promise<{ success: boolean; message: string }> {
    return api.post(`/13f/execute-basket/${basketId}`).then(res => res.data)
  },

  get13FBaskets(): Promise<{ baskets: PortfolioBasket[] }> {
    return api.get('/13f/baskets').then(res => res.data)
  },
  ...
  async setLLMProvider(provider: LLMProvider): Promise<LLMProviderResponse> {
