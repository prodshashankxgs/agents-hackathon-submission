...
export class OptimizedParsingService {
  ...
  private async parseWithRules(input: string, startTime: number): Promise<OptimizedParseResult> {
    ...
    console.log(`⚠️ Rule-based parsing failed (confidence: ${parseResult.confidence.toFixed(3)}), trying semantic cache...`);
  ...
  private async parseWithLLM(input: string, startTime: number, classification: ClassificationResult): Promise<OptimizedParseResult> {
    ...
    // Use existing OpenAI service with potential model override
    const originalModel = (this.openaiService as any).model;
    if (selectedModel.name !== 'gpt-4o-mini') {
      // Note: This would require modifying OpenAIService to support model selection
      console.log(`📝 Model override recommended: ${selectedModel.name}`);
    }

    const intent = await this.openaiService.parseTradeIntent(input);
    const processingTime = Date.now() - startTime;
