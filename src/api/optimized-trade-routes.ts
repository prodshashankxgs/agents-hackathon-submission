 ...
 * 
 * Features:
 * - Fast parsing with multiple tiers (OpenAI + Claude support)
...
export class OptimizedTradeRoutes {
  ...
  private setupRoutes(): void {
    ...
    this.router.get('/llm/provider', this.handleGetLLMProvider.bind(this));
    this.router.post('/llm/provider', this.handleSetLLMProvider.bind(this));
  }
  ...
  */
   ...
   */
  private async handleGetLLMProvider(req: express.Request, res: express.Response): Promise<void> {
    try {
      const provider = optimizedParsingService.getLLMProvider();
      ...
      this.handleError(res, error, 'LLM_PROVIDER_GET_FAILED');
    }
  }
   ...
   * Set LLM provider
   */
  private async handleSetLLMProvider(req: express.Request, res: express.Response): Promise<void> {
    try {
      ...
      const previousProvider = optimizedParsingService.getLLMProvider();
      ...
      res.json({
        ...
        message: `Switched from ${previousProvider} to ${provider}`,
        timestamp: new Date().toISOString()
      });
    } catch (error) {
      this.handleError(res, error, 'LLM_PROVIDER_SET_FAILED');
    }
  }
